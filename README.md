...
Интеграционный сервер, использующий Model Context Protocol (MCP), для предоставления доступа к данным и функциям Bitrix24 для больших языковых моделей (LLM) и других AI-агентов.

## Мотивация

Этот проект позволяет легко "подключить" ваш портал Bitrix24 к AI-системам. LLM могут безопасно взаимодействовать с вашими CRM-данными (контакты, сделки), используя предопределенные "инструменты" и "ресурсы", предоставляемые этим сервером через стандартный протокол MCP.
...
Проект следует принципам Чистой Архитектуры для обеспечения слабой связанности и высокой тестируемости:

```
app/
├── domain/              # Бизнес-логика, сущности, интерфейсы репозиториев
│   ├── entities/        # Сущности (Contact, Deal, etc.)
│   ├── interfaces/      # Абстрактные репозитории
│   └── bitrix_types.py  # Пользовательские типы данных Bitrix24
├── application/         # Слой приложения: Use Cases / Services
│   └── services/        # Координация и логика приложения
├── infrastructure/      # Внешние зависимости: DB, API клиенты, Frameworks
│   ├── bitrix/          # Реализация репозиториев для Bitrix24 API
│   │   └── mixins/      # Миксины для Bitrix24 API (CRUD, Batch, etc.)
│   ├── logging/         # Настройка логирования (structlog)
│   ├── mcp/             # Реализация и настройка MCP сервера
│   └── ioc.py           # Контейнер зависимостей (wireup)
└── presentation/        # Входная точка, UI, API контроллеры
    └── mcp.py           # Конфигурация и запуск MCP сервера
```
